 ...
 *a: Application   : LFS                                                  *
 *a:                                                                      *
 *a: Program name  : /ec/prod/server/sas/1lfs/listings/bachfab/test1_csv_sumrec.sas            a*
 **************************************************************************
 *a: Description   :                                                      *

 **************************************************************************
 *a: Output file(s): /ec/prod/server/sas/1lfs/listings/bachfab/test1_csv_sumrec.csv            a*
 *a: -------------------------------------------------------------------- *
 *b:                                                                      *
...
/* $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
data work.temp(keep= AGE POP SUMREC COUNTRY YEAR QUARTER
...
class WSTATOR  / mlf preloadfmt order=data;
    ;var POP SUMREC;
       output   out=work.RESONE sum(POP)=POP sum(SUMREC)=SUMREC ;
...
%MACRO final;
 %let fname=/ec/prod/server/sas/1lfs/listings/bachfab/test1_csv_sumrec.csv           ;
...
AGE COUNTR1Y COUNTRYB REGION REGIONW SEX WSTATOR FREQ POP SUMREC FLAG FLAG2 wgt ckey noise freq2 value2 AD1 RAD1 DR1 AD2 RAD2 DR2 ckey2 pcv
;
...
/*	rseed = 1 + mod(SUMREC,2147483647);
	call ranuni(rseed, ckey2);
	ckey2 = mod(SUMREC*397204094,2**31-1)/(2**31-1);
*/	ckey2 = mod(SUMREC*6364136223846793005+1442695040888963407,2**64)/(2**64);
	ckey2 = int(&m*ckey2+1);
	pcv = min(freq,&pmod);
 %end;
...
%if %sysfunc(exist(work.RESALL)) %then %do;
  ...
  proc export data=work.RESALL outfile="&fname "
    ...
    delimiter=',';
  quit;
%sysexec chmod 777 /ec/prod/server/sas/1lfs/listings/bachfab/test1_csv_sumrec.csv  ;
/* libname libuser "/ec/prod/server/sas/1lfs/listings/bachfab";			*/
/* data libuser.test1_csv_sumrec  ; set work.RESALL;run;*/
/* %sysexec chmod 777 /ec/prod/server/sas/1lfs/listings/bachfab/test1_csv_sumrec.sas7bdat  ;*/
...
%MEND allfiles;
proc printto log = "/ec/prod/server/sas/1lfs/listings/bachfab/test1_csv_sumrec.log" new;
quit;
